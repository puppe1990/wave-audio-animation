...
class TestLoadAudio:
    ...
    def test_load_file_path_object(self, audio_service: AudioService, sine_wave_file: Path):
        audio = audio_service.load_audio(sine_wave_file)
        assert isinstance(audio, AudioSegment)

    def test_load_file_path_string(self, audio_service: AudioService, sine_wave_file: Path):
        audio = audio_service.load_audio(str(sine_wave_file))
        assert isinstance(audio, AudioSegment)


class TestExtractAmplitudes:
    def test_returns_correct_structure(self, audio_service: AudioService, sine_wave_file: Path):
        audio = audio_service.load_audio(sine_wave_file)
    ...
    def test_frame_count_matches_expected(self, audio_service: AudioService, sine_wave_file: Path):
        audio = audio_service.load_audio(sine_wave_file)
        result = audio_service.extract_amplitudes(audio)
    ...
    def test_amplitudes_normalized(self, audio_service: AudioService, sine_wave_file: Path):
        audio = audio_service.load_audio(sine_wave_file)
        result = audio_service.extract_amplitudes(audio)

        amplitudes = result["amplitudes"]
        assert len(amplitudes) > 0
        assert all(0.0 <= a <= 1.0 for a in amplitudes), "All amplitudes must be in [0, 1]"
        assert max(amplitudes) == pytest.approx(1.0), "Max amplitude must be 1.0 after normalization"
    ...
    def test_silent_audio_produces_near_zero_amplitudes(
        ...
        amplitudes = result["amplitudes"]
        assert all(a == 0.0 for a in amplitudes), "Silent audio should produce all-zero amplitudes"
    ...
    def test_sine_wave_produces_non_zero_amplitudes(
        ...
        amplitudes = result["amplitudes"]
        assert any(a > 0.0 for a in amplitudes), "Sine wave should produce non-zero amplitudes"

    def test_duration_matches_audio(self, audio_service: AudioService, sine_wave_file: Path):
        audio = audio_service.load_audio(sine_wave_file)
        result = audio_service.extract_amplitudes(audio)
    ...
    def test_sample_rate_matches_audio(self, audio_service: AudioService, sine_wave_file: Path):
        audio = audio_service.load_audio(sine_wave_file)
        result = audio_service.extract_amplitudes(audio)
